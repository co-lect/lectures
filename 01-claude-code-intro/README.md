...
**DRAFT v4** — 全 12 枚。**主張 → 具体理由** の流れに再編。「PC を AI が操作する道具」フレームに統一。
...
| # | チャプター | 内容 |
|---|---|---|
| 01 | — | Cover — PC を AI が操作する時代へ |
| 02 | — | このドキュメントの読み方（HTML/AI 共読・概要のみ宣言） |
| 03 | Ch.1 | **誰のためか — エンジニアだけの話ではない**（先頭の主張） |
| 04 | Ch.2 | AI コーディング 4 年史（ブラウザ→エディタ→PC 全体） |
| 05 | Ch.3 | 立ち位置の変化（PC を操作する人 → 指示する人） |
| 06 | Ch.4 | 拡張性と並列性（Claude Code の決定打） |
| 07 | Ch.5 | 何ができるか — 4 象限の実例 |
| 08 | Ch.6 | 実績① 数字（ARR $30B、評価額 $380B、SWE-bench 87.6%） |
| 09 | Ch.6 | 実績② 顧客（Deloitte 47 万人、Salesforce、Altana 等） |
| 10 | Ch.7 | 結論 — 数年後、これが標準になる |
| 11 | Ch.8 | シリーズ予告（02〜07） |
| 12 | Coda | 終わりに — 手段は揃った／目的の時代 + 参考 3 チャンネル |

### v4 での主な変更

- **誰のためか を Slide 8 → Slide 3 に移動**（主張先行）
- **5 製品ランドスケープ削除**（Copilot/Cursor は別カテゴリなので比較を整理）
- 全体を 13 → 12 枚に圧縮、章番号・ページ番号を調整
